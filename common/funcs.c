...
#define GET_FUNCTION(dll, name)                                                                                        \
    HackAssert(NT_SUCCESS(LdrGetProcedureAddress((dll), &(ANSI_STRING)RTL_CONSTANT_STRING(#name), 0, &__imp_##name)))
...
static void GetLdrGetProcedureAddress(void)
{
    ...
    HackAssert(ntHeaders->Signature == IMAGE_NT_SIGNATURE);
    ...
    HackAssert(__imp_LdrGetProcedureAddress != NULL);
}
